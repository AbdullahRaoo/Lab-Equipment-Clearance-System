...
export default async function Lab2Page() {
  ...
  const { data: inventory, error: invError } = await getLabInventory('lab2');
  const { data: issues, error: issuesError } = await getLabIssues('lab2');
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <h1 className="text-2xl font-bold text-gray-900">Lab 2 - Inventory Management</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Link
            href="/labs/lab2/inventory"
            className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow"
          >
            <h3 className="text-lg font-semibold mb-2">View Inventory</h3>
            <p className="text-gray-600 text-sm">Browse all equipment in Lab 2</p>
          </Link>

          <Link
            href="/labs/lab2/issues"
            className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow"
          >
