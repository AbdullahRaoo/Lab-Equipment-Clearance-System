...
import { LAB_NAMES, LabSchema } from '@/types/clearance';
...
export function LabAdminDashboard({ user }: LabAdminDashboardProps) {
    const assignedLabs = user.assigned_labs || [];
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-6">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">Manage inventory and clearance for your assigned labs</p>
                    </div>
                ...
                <div className="mt-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        >
                            ...
                            <p className="text-sm text-gray-600">Review pending clearance requests from students</p>
                        </Link>

                        <div className="bg-blue-50 p-6 rounded-lg border border-blue-100 hover:border-blue-300 transition-colors">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <h3 className="font-semibold text-gray-900">Add Inventory</h3>
                            </div>
                            <p className="text-sm text-gray-600">Register new equipment in your labs (Coming Soon)</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">My Assigned Labs</h3>
                {assignedLabs.length === 0 ? (
                    <div className="bg-yellow-50 p-4 rounded-md border border-yellow-200 text-yellow-800">
                        You have not been assigned to any labs yet. Please contact the System Administrator.
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {assignedLabs.map((labKey: string) => {
                            const name = LAB_NAMES[labKey as LabSchema] || labKey;
                            return (
                                <Link
                                    key={labKey}
                                    href={`/labs/${labKey}`}
